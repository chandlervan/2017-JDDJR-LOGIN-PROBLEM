...
# device_ip_city_dict = {}
# login_len = len(df[df.buy == 0])
# for ii in range(len(df[df.buy == 0])):
#     if str(int(device[ii]))+'-'+str(int(ip[ii])) + '-'+str(int(city[ii])) in device_ip_city_dict.keys():
#         device_ip_city_dict[str(int(device[ii]))+'-'+str(int(ip[ii])) + '-'+str(int(city[ii]))] +=1
#     else:
#         device_ip_city_dict[str(int(device[ii]))+'-'+str(int(ip[ii])) + '-'+str(int(city[ii]))] = 1
# for key in device_ip_city_dict:
#     device_ip_city_dict[key] = round(device_ip_city_dict[key]/login_len,6)
...
def generate_feature(id):
    ...
    for row in df_buy.itertuples():
        ...
        if cnt_total == 0:
            ...
            # combine_feature = device_ip_city_dict[str(int(df_login_1.device.tolist()[0]))+'-'+str(int(df_login_1.ip.tolist()[0]))+'-'+str(int(df_login_1.city.tolist()[0]))]
            ...
            # combine_feature = device_ip_city_dict[str(int(df_login_1.device.tolist()[0]))+'-'+
            #                                       str(int(df_login_1.ip.tolist()[0]))+'-'+str(int(df_login_1.city.tolist()[0]))]
            if df_login_1.time.tolist()[0].weekday()> 4:
                weekday = 1
            else:
                weekday = 0
        ...
        feature.append([rowkey,is_risk,is_train,buy_time,login_time,is_scan_1,log_from_1,log_type_1,
                        ...
                        timelong_diff_avg_10,weekday,df_login_5_min,df_login_30_min,device_rate,ip_rate,city_rate])
    return feature
...
if __name__ == '__main__':
    ...
    df_feature.columns = ['rowkey','is_risk','is_train','buy_time','login_time','is_scan_1','log_from_1','log_type_1',
                          ...
                          'city_rate']
    # df_feature['login_time'] = df_feature['login_time'].fillna(9)
    # df_feature['log_from_1'] = df_feature['log_from_1'].fillna(1)
    # df_feature['log_result_1'] = df_feature['log_result_1'].fillna(1)
    # df_feature['log_type_1'] = df_feature['log_type_1'].fillna(df_feature['log_type_1'].mode())
    # for col in ['timelong_1','buy_login_period_1','city_rate_3','city_rate_6','city_rate_10',
    #              'device_rate_3','device_rate_6','device_rate_10','ip_rate_3','ip_rate_6','ip_rate_10',
    #             'result_rate_3','result_rate_6','result_rate_10','device_chg_rate_3','device_chg_rate_6',
    #             'device_chg_rate_10','city_chg_rate_3','city_chg_rate_6','city_chg_rate_10','ip_chg_rate_3',
    #             'ip_chg_rate_6','ip_chg_rate_10','timelong_diff_avg_3','timelong_diff_avg_6',
    #             'timelong_diff_avg_10']:
    #              df_feature[col] = df_feature[col].fillna(df_feature[col].mean())
    ...
    feature_all_one_hot_not_none = feature_all_one_hot[feature_all_one_hot.login_time.notnull()]
    feature_all_one_hot_none = feature_all_one_hot[feature_all_one_hot.login_time.isnull()]
    feature_all_one_hot_not_none.to_csv('feature_1207_not_null.csv')
    feature_all_one_hot_none.to_csv('feature_1207_null.csv')
